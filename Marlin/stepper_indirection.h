 ...
 */
  ...
  #define REV_E_DIR() do{ switch (current_block->active_extruder) { case 0: E0_DIR_WRITE(INVERT_E0_DIR); break; case 1: E1_DIR_WRITE(INVERT_E1_DIR); break; case 2: E2_DIR_WRITE(INVERT_E2_DIR); break; case 3: E3_DIR_WRITE(INVERT_E3_DIR); } }while(0)
    ...
    #define E_STEP_WRITE(v) do{ if (extruder_duplication_enabled) { E0_STEP_WRITE(v); E1_STEP_WRITE(v); } else if (current_block->active_extruder == 0) { E0_STEP_WRITE(v); } else if (current_block->active_extruder == 1) { E1_STEP_WRITE(v); } else { E2_STEP_WRITE(v); } }while(0)
    #define NORM_E_DIR() do{ if (extruder_duplication_enabled) { E0_DIR_WRITE(!INVERT_E0_DIR); E1_DIR_WRITE(!INVERT_E1_DIR); } else if (current_block->active_extruder == 0) { E0_DIR_WRITE(!INVERT_E0_DIR); } else if (current_block->active_extruder == 1) { E1_DIR_WRITE(!INVERT_E1_DIR); } else { E2_DIR_WRITE(!INVERT_E2_DIR); } }while(0)
    #define REV_E_DIR() do{ if (extruder_duplication_enabled) { E0_DIR_WRITE(INVERT_E0_DIR); E1_DIR_WRITE(INVERT_E1_DIR); } else if (current_block->active_extruder == 0) { E0_DIR_WRITE(INVERT_E0_DIR); } else if (current_block->active_extruder == 1) { E1_DIR_WRITE(INVERT_E1_DIR); } else { E2_DIR_WRITE(INVERT_E2_DIR); } }while(0)
  #else
